...
def collect_data(duration_sec: float,
    ...
    #have spectator follow the vehicle
    ...
    for _ in range(frame_count):
        ...
        #calculating the vehicle's velocity
        v = vehicle.get_velocity()
        speed = (v.x**2 + v.y**2 + v.z**2) ** 0.5

        #image converstion to rgb 244x244
    ...
    np.save(Path(output_dir) / f"{output_prefix}_velocity.npy", np.array(velocities))
